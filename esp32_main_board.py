...
try:
    # Use the SPI pins from the user's working test code
    spi = SPI(
        2,
        baudrate=20000000,
        polarity=0,
    ...
    # Try ST7735 driver first since user confirmed it works
    try:
        import st7735
        # The user's test code: tft = st7735.TFT(spi, 2, 4, 5)  # dc=2, rst=4, cs=5
        tft = st7735.TFT(spi, PIN_TFT_DC, PIN_TFT_RST, PIN_TFT_CS)
        tft.initr()
        tft.rgb(True)
        # Fix laterally inverted text by rotating or flipping MADCTL
        # 0=portrait, 1=landscape, 2=portrait reverse, 3=landscape reverse
        # Try rotation 1 or 3 for landscape, or just 0 for default
        try:
            tft.rotation(1) # Assuming landscape orientation is desired
        except:
            pass
